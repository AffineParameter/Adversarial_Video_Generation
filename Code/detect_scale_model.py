...
class DetScaleModel:
    """
    A DetScaleModel is a network that takes as input one video frame and attempts to discriminate
    ...
    def define_graph(self):
            ...
            with tf.name_scope('convolutions'):
                ...
                for i in range(len(self.kernel_sizes)):
                    ...
                    last_out_height = conv_out_size(
                        last_out_height, c.PADDING_DET, self.kernel_sizes[i], 1)
                    last_out_width = conv_out_size(
                        last_out_width, c.PADDING_DET, self.kernel_sizes[i], 1)
        ...
        def generate_predictions():
            ...
            with tf.name_scope('calculation'):
                preds = tf.zeros([self.batch_size, 3])
                ...
                with tf.name_scope('convolutions'):
                    for i in range(len(conv_ws)):
                        ...
                        preds = tf.nn.conv2d(
                            last_input, conv_ws[i], [1, 1, 1, 1],
                            padding=c.PADDING_DET)
        ...
        # Generate Last FC Layer Not "predictions" like in the d_scale_model
        # model
        self.last_fc = generate_predictions()
        ##
        # Training handled by DetectionModel
        ##
